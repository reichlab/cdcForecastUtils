...
```{r}
submission_df <- trajectories_to_binned_distributions(
...
# Throwing errors!
#
#cdcForecastUtils::verify_entry(
#  submission_df %>% mutate(type = tolower(type)),
#  challenge = "state_ili")

generate_csv_from_submission_df(submission_df %>% mutate(type = tolower(type)),"./")
```
